...
function buildSubjectAverageChart(chartData, seriesLabel) {
    if (!chartData?.series?.length) return { labels: [], series: [] };
...
function initBottomNav() {
    ...
    if (isLoginPage) {
        ...
        if (languageSwitch) languageSwitch.parentElement.style.display = 'none';
        return;
    }
    ...
    if (languageSwitch) languageSwitch.parentElement.style.display = '';
    ...
    try {
        ...
        const subjectAverages = subjects
            .map(subject => {
                ...
                const subjectResults = resultsBySubject[subjectId] || [];
                if (!subjectResults.length) return null;
                const avg = Math.round(subjectResults.reduce((sum, r) => sum + (r.score || 0), 0) / subjectResults.length);
                return {
                    label: lang === 'uz' ? subject.nameUz : subject.nameRu,
                    average: avg
                };
            })
            .filter(Boolean);

        const subjectChartData = subjectAverages.length
            ? {
                labels: subjectAverages.map(item => item.label),
                series: [{
        ...
        <div class="card" id="interestTestContainer">
            ...
            <div id="testResults" style="display: none;">
                <div style="text-align: center; margin-bottom: 2rem;">
                    <h2 style="color: var(--text-primary); margin-bottom: 0.5rem; font-size: 1.75rem;">
                ...
                <div style="max-width: 700px; margin: 0 auto 2rem; padding: 1.5rem; background: var(--bg-tertiary); border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
    ...
    try {
        ...
        const loadClassAnalytics = async (value) => {
            ...
            try {
                ...
                if (timelineRes.success) {
                    const chartData = buildSubjectAverageChart(timelineRes.data, lang === 'uz' ? 'O\'rtacha ball' : 'Средний балл');
...
async function renderTeacherStudentProfile({ studentId }) {
    ...
    try {
        ...
        if (timelineRes.success) {
            const chartData = buildSubjectAverageChart(timelineRes.data, lang === 'uz' ? 'Natija' : 'Результат');
